...
def reduce_regions_hierarchically(chips):
    ...
    -------
    :py:class:`set`
        Set of 32-bit region codes which minimally cover the set of given
        chips.
    """
    t = RegionTree()
    for (x, y) in chips:
...
class RegionTree(object):
    ...
    def get_regions(self):
        """Get a set of integer region representations.

        Returns
        -------
        :py:class:`set`
            Set of 32-bit region codes as might be generated by
        ...
        if self.locally_selected != set():
            ...
            for e in self.locally_selected:
                elements |= 1 << e
            regions.add(region_code | elements)
